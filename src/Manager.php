...
class Manager
{
    ...
    private function getTranslationsFromFiles()
    {
         ...
         */
        $functions = config('translation-gui.localization_methods', ['__']);
    ...
    {
        if (! $this->disk->exists(storage_path('translation-gui'))) {
            $this->disk->makeDirectory(storage_path('translation-gui'));

            $this->disk->put(storage_path('translation-gui'.'/.gitignore'), "*\n!.gitignore");
        }

        $this->disk->copyDirectory(resource_path('lang'), storage_path('translation-gui/'.time()));
    }
}
